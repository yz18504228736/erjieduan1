            ...
            <table align="center" width="100%">
                ...
                <c:forEach var="item" items="${sessionScope.cart}">
                    <tr height="60">
                        ...
                        <td align="center" width="20%">
                            <input type="text" value="${item.value.num}" size="3" lang="3" onblur="alterSorder(this, ${item.key})"/>
                        </td>
                        <td align="center" width="20%">￥${item.value.price * item.value.num}</td>
                        <td align="center" width="20%">
                            <input type="button" value="删除" class="btn_next" lang="3" onclick="removeSorder(this,${item.key})" />
                        </td>
                </tr>
                </c:forEach>
                ...
                <tr>
                    <td colspan="6" align="right">总计:
                        <span style="font-size:36px;">&yen;&nbsp;${totalMoney}</span>
...
<script>
    function removeSorder(obj, foodId) {
        window.location.href = '${ctx}/foodController?op=remove_cart&foodId='+foodId+'&typeId=${typeId}&tableId=${tableId}'
    }
    function alterSorder(obj, foodId) {
        let num = $(obj).val();
        if (num < 1) {
            return ;
        }
        window.location.href = '${ctx}/foodController?op=change_cart&foodId='+foodId+'&typeId=${typeId}&tableId=${tableId}&num='+num;
    }
